...
### Requirement: 合成回复回显参数文件落地位置

`record()` SHALL 返回落地位置描述（storage 后端路径，或回退写入的临时文件绝对路径）；监听器 SHALL 把该位置追加到合成回复（`→ params: <location>`），使使用者能直接定位实际生成的参数文件，避免误判为"未输出临时文件"。

#### Scenario: 回复中给出临时文件绝对路径

- **GIVEN** `storage` 在宿主根插件不可达（走 `os.tmpdir` 回退）
- **WHEN** 调试模式拦截一次 `llm/stream`
- **THEN** 合成回复文本包含 `→ params: <os.tmpdir 绝对路径>/dsh-llm-debug-<uuid>.json`
- **AND** 该路径对应的文件确实存在
...
- `reply: string` —— 拦截后返回的合成助手消息文本，默认 `[DEBUG] LLM call blocked; request params recorded to a temp file.`（运行时追加 `→ params: <location>`）。
